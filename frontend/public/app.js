...
function hideLegacyChrome(){const root=document.getElementById('o2vr-container'); if(!root) return; root.querySelectorAll('.ggskin_button').forEach(n=>n.style.display='none'); const logo=root.querySelector('img[src$="logo_01.png"]'); if(logo) logo.style.display='none';}

async function loadModel(model){if(!model){setStatus('Geen model geselecteerd.');return} const token=++loadToken; setStatus('Model laden…'); const p=createPlayer(); try{p.readConfigUrl(model.xml); if(model.id==='fillpak_classic' && typeof window.object2vrSkin==='function'){ try{ new window.object2vrSkin(p,''); hideLegacyChrome(); }catch(_){ /* ignore skin errors */ }} if(token!==loadToken) return; setStatus(`${CATALOG[currentCat].label} – ${model.label}`)}catch(e){console.error(e); if(token!==loadToken) return; setStatus('Kon viewer niet initialiseren.','error'); renderFallback('Fout bij initialiseren van viewer.');}}
